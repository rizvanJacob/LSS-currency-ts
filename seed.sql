...
INSERT INTO Category (name) VALUES ('BWC'), ('F0'), ('F2'), ('F3'),('F5'),('F9'),('FWC'),('FHQ'),('FP2'),('FP5'),('FE'),('H0'),('H3'),('H4'),('H5'),('H6'),('H7'),('HHQ'),('HO'),('HB'),('HPV'),('TG'),('TM'),('TF'),('TC'),('APU'),('HQ 8071'),('Medical')
...
INSERT INTO CategoryToRequirement (category, requirement) SELECT category.id, requirement.id FROM Category AS category CROSS JOIN Requirement AS requirement WHERE category.name IN ('BWC','F0','F2','F3','F5','F9','FWC','FHQ','FP2','FP5','FE') AND requirement.name IN ('APT (F)', 'Dgy (F)', 'SVT', 'NVG R (F)', 'DFS (R)', 'DFS (Y)', 'EST');
INSERT INTO CategoryToRequirement (category, requirement) SELECT category.id, requirement.id FROM Category AS category CROSS JOIN Requirement AS requirement WHERE category.name IN ('H0', 'HPV') AND requirement.name IN ('APT (H)', 'Dgy (T/H)', 'SVT', 'HUET');
INSERT INTO CategoryToRequirement (category, requirement) SELECT category.id, requirement.id FROM Category AS category CROSS JOIN Requirement AS requirement WHERE category.name IN ('H3','H4','H5','H6','H7','HHQ','HO','HB') AND requirement.name IN ('APT (H)', 'Dgy (T/H)', 'SVT', 'HUET', 'NVG R (H)');
INSERT INTO CategoryToRequirement (category, requirement) SELECT category.id, requirement.id FROM Category AS category CROSS JOIN Requirement AS requirement WHERE category.name IN ('TG','TM','TF', 'HQ 8071')  AND requirement.name IN ('APT (T)', 'Dgy (T/H)','SVT');
INSERT INTO CategoryToRequirement (category, requirement) SELECT category.id, requirement.id FROM Category AS category CROSS JOIN Requirement AS requirement WHERE category.name IN ('TC') AND requirement.name IN ('APT (T)', 'Dgy (T/H)','SVT', 'NVG R (F)');
INSERT INTO CategoryToRequirement (category, requirement) SELECT category.id, requirement.id FROM Category AS category CROSS JOIN Requirement AS requirement WHERE category.name IN ('APU') AND requirement.name IN ('APT (T)', 'Dgy (T/H)','SVT', 'HUET');
