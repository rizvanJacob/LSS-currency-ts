...
INSERT INTO CategoryToRequirement (category, requirement) SELECT category.id, requirement.id FROM Category AS category CROSS JOIN Requirement AS requirement WHERE category.name IN ('TG','TM','TF', 'HQ 8071')  AND requirement.name IN ('ACE', 'APT (T)', 'Dgy (T/H)','SVT');
